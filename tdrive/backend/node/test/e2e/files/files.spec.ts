...
describe("The Files feature", () => {
  ...
  describe("On user send files", () => {
    ...
    it("should fail an upload POST when the backend storage fails", async () => {
      ...
      const writeS3Mock = jest
        .spyOn(MinioClient.prototype, "putObject")
        .mockImplementation(thrower);
      const writeLocalMock = jest
        .spyOn(LocalConnectorService.prototype, "write")
        .mockImplementation(thrower);
